...
for {
select { case <-ctx.Done(): return nil; case up := <-updates:
...
f := atoi(parts[2]); s := atoi(parts[3]); a := atoi(parts[4]); R := atof(parts[5])
...
p := b.engPos()
return fmt.Sprintf("Mode: paper\nEquity: %.2f USD\nPos: %s qty=%.4f entry=%.2f unrl=%.2f", b.engEq(), actName(p.Side), p.Qty, p.Entry, p.Unreal)
}


func (b *Bot) engEq() float64 { return b.engSnapshot(0).EquityUSD }
func (b *Bot) engPos() core.Position { return b.engSnapshot(0).Position }
func (b *Bot) engSnapshot(px float64) core.AccountState { return b.engSnapshot(px) }


func (b *Bot) reply(bot *gobot.BotAPI, chatID int64, text string) {
msg := gobot.NewMessage(chatID, text)
if _, err := bot.Send(msg); err != nil { log.Error().Err(err).Msg("send tg msg") }
}


func actName(a core.Action) string { switch a { case core.Buy: return "LONG"; case core.Sell: return "SHORT"; default: return "FLAT" } }
