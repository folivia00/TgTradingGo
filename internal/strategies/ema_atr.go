...
if len(s.buf) < s.Warmup() { return core.Signal{Action: core.None}, nil }
...
k := 2.0 / (float64(n) + 1)
res[0] = x[0]
for i := 1; i < len(x); i++ { res[i] = x[i]*k + res[i-1]*(1-k) }
return res
}
...
func closes(k []core.Kline) []float64 { r := make([]float64, len(k)); for i:=range k { r[i]=k[i].Close }; return r }
func highs(k []core.Kline) []float64 { r := make([]float64, len(k)); for i:=range k { r[i]=k[i].High }; return r }
func lows(k []core.Kline) []float64 { r := make([]float64, len(k)); for i:=range k { r[i]=k[i].Low }; return r }


func max3(a,b,c int) int { if a<b {a=b}; if a<c {a=c}; return a }
func max3f(a,b,c float64) float64 { if a<b {a=b}; if a<c {a=c}; return a }